...
def initialize_dvc() -> None:
    ...
    print(run_shell_command("dvc init"))
    ...
    print(run_shell_command("git commit -nm 'Initialized DVC - done =========='"))
...
def initialize_dvc_storage(dvc_remote_name: str, dvc_remote_url: str) -> None:
    if not run_shell_command("dvc remote list"):
        ...
        run_shell_command(f"git add .dvc/config")
        run_shell_command(
            f"git commit -nm 'Configured remote storage at: {dvc_remote_url}'"
        )
    else:
        DATA_UTILS_LOGGER.info("DVC storage already intialized ... ")
...
def commit_to_dvc(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    ...
    run_shell_command(
        f"git commit -nm 'Updated version of the data from v{current_version} to {next_version}'"
    )
    ...
    # run_shell_command(f"git push --follow-tags")
    # run_shell_command(f"git push -f --tags")
